...
export default async function StoryPage({ params }: PageProps) {
  ...
  return <article className="story-page">
    ...
    <StructuredData data={{ "@context": "https://schema.org", "@type": "TouristTrip", "@id": `${pageUrl}#tour`, name: `${route.title}: ${route.course}`, description: route.seoDescription, url: pageUrl, image: route.image ? `${getSiteUrl()}/images/${route.image}.webp` : `${pageUrl}/share-image`, touristType: "Korean drama and film fans", itinerary: { "@type": "ItemList", numberOfItems: route.stops.length, itemListElement: route.stops.map((stop, index) => ({ "@type": "ListItem", position: index + 1, item: { "@type": "Place", name: stop.name, alternateName: stop.ko, description: stop.scene, url: `${pageUrl}#stop-${index + 1}` } })) } }} />
