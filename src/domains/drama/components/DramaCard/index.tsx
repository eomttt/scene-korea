...
export function DramaCard({ route, priority = false, collectionSearch = "", onNavigate }: { route: DramaCardData; priority?: boolean; collectionSearch?: string; onNavigate?: () => void }) {
  return <article className="drama-card"><Link href={`/stories/${route.id}${collectionSearch}`} onNavigate={onNavigate} className="card-link" aria-label={`Explore ${route.course} from ${route.title}`}>
    ...
    <div className="card-copy"><p className="card-title">{route.title} <span>· {route.format}</span></p><h3>{route.course}</h3><p className="card-hook">{route.hook}</p><div className="card-meta"><span><Clock3 size={14} aria-hidden="true" />{route.duration}</span><span>{route.stopNames.length} {route.stopNames.length === 1 ? "stop" : "stops"} <ArrowUpRight size={16} aria-hidden="true" /></span></div></div>
