...
export function DramaCard({ route, priority = false, collectionSearch = "" }: { route: DramaRoute; priority?: boolean; collectionSearch?: string }) {
  return <article className="drama-card"><Link href={`/stories/${route.id}${collectionSearch}`} className="card-link" aria-label={`Explore ${route.course} from ${route.title}`}>
