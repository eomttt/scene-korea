...
export function DramaCollection({ routes }: { routes: DramaRoute[] }) {
  ...
  const matches = routes.filter((route) => normalizeTitle(`${route.title} ${route.ko}`).includes(search));
  ...
  return <section id="collection" aria-labelledby="collection-heading">
    ...
    <div className="collection-tools"><div className="search-field"><Search size={19} aria-hidden="true" /><label className="sr-only" htmlFor="drama-search">Search drama titles</label><input id="drama-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search a drama title, in English or Korean" />{query ? <button type="button" aria-label="Clear search" onClick={() => setQuery("")}><X size={18} aria-hidden="true" /></button> : null}</div><p role="status">{matches.length} {matches.length === 1 ? "story" : "stories"}</p></div>
